...
export const ProfileHeader: React.FC<ProfileHeaderProps> = ({
  ...
  // Update parent component when posts count changes - fixed version
  useEffect(() => {
    if (onPostsCountUpdateRef.current && postsCount !== undefined) {
      onPostsCountUpdateRef.current(postsCount);
    }
  }, [postsCount]); // Only depend on postsCount
              ...
              <div ref={statsRef} className="border-t border-gray-200/50 dark:border-gray-800/50 pt-8">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-8">
                  ...
                  {profile.role === 'author' && (
                    ...
                    >
                      <div className="flex items-center justify-center gap-2 mb-2">
                        <BookOpen size={20} className="text-gray-400 group-hover:text-[#EF3866] transition-colors" />
                        {postsCountLoading && (
                          <div className="animate-spin rounded-full h-3 w-3 border border-gray-400 border-t-transparent"></div>
                        )}
                      </div>
                      <div className="font-light text-2xl sm:text-3xl text-gray-900 dark:text-white group-hover:text-[#EF3866] transition-colors tracking-tight">
                        {postsCountLoading ? '...' : postsCount}
                      </div>
                      <div className="text-sm text-gray-600 dark:text-gray-400 font-medium uppercase tracking-wide">
                        Posts
                      </div>
