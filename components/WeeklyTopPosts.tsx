...
const WeeklyTopPosts = () => {
  ...
  const getRankBadgeColor = (index: number) => {
    switch (index) {
      ...
      default:
        return 'bg-gradient-to-r from-[#EF3866] to-pink-600';
    }
  };
            ...
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="border-b border-gray-100 dark:border-gray-800 p-4 md:p-6 last:border-b-0">
                <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
                  ...
                  <div className="w-full sm:w-48 md:w-56 lg:w-64 xl:w-72 h-32 sm:h-36 bg-gray-200 dark:bg-gray-700 rounded-lg"></div>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex items-center">
          <button
            onClick={refreshPosts}
            disabled={loading}
            className="flex items-center gap-2 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            title="Refresh"
          >
      ...
      {posts.length > 0 ? (
        <div className="space-y-8">
          <div className="space-y-0 bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
            {posts.map((post, index) => (
              ...
              >
                {/* Rank Badge - Positioned to not overlap with categories */}
                ...
                <Link href={`/post/${post._id}`} className="block">
                  <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
                    ...
                    <div className="flex-1 min-w-0 order-2 sm:order-1">
                        ...
                        {post.categories && post.categories.length > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {post.categories.slice(0, 2).map((cat, catIndex) => (
                              <span
                                key={catIndex}
                                className="inline-flex items-center gap-1 bg-[#EF3866]/10 text-[#EF3866] text-xs font-medium px-3 py-1 rounded-full hover:bg-[#EF3866]/20 transition-colors duration-200"
                              >
                      ...
                      {/* Categories for mobile (when rank badge is positioned differently) */}
                      {post.categories && post.categories.length > 0 && (
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400 mb-4">
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          <span className="text-xs sm:text-sm">
                            {getTimeAgo(post.publishedAt || post._createdAt)}
                          </span>
                        </div>
                      </div>
          ...
          {pagination?.has_more && (
            <div className="text-center">
              <button
                ...
                disabled={loading}
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#EF3866] text-white rounded-lg hover:bg-[#d63384] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
        ...
        <div className="text-center py-12 bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-800">
          ...
          <p className="text-gray-500 dark:text-gray-500 mb-6">
            There are no trending posts for this week yet.
          </p>
          <button
            onClick={refreshPosts}
            className="inline-flex items-center gap-2 px-4 py-2 bg-[#EF3866] text-white rounded-lg hover:bg-[#d63384] transition-colors duration-200"
          >
            <RefreshCw className="w-4 h-4" />
            Refresh
          </button>
