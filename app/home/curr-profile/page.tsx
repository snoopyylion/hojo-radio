...
import { TrendingUp, CheckCircle, Crown, BarChart3, Edit3, MessageCircle, Bookmark, Activity } from "lucide-react";
...
export default function UserDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'profile-details' | 'posts' | 'comments' | 'verified' | 'my-posts' | 'activity' | 'author' | 'bookmarks'>('overview');
      ...
      try {
        ...
        if (profileResponse.ok) {
          ...
          const newProfile: UserProfile = {
            ...
            cover_image_url: profileData.cover_image_url || undefined, // Get from API data
  ...
  // Define styles - Updated to remove top padding
  const styles = {
    container: "min-h-screen bg-gray-50/50 dark:bg-gray-900/20 pb-[100px]", // Removed pt-[100px]
    profileContainer: "max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8",
    contentArea: "flex-1 flex flex-col gap-6 max-w-7xl mx-auto",
  ...
  return (
    <div className={styles.container} ref={containerRef}>
      ...
      <div className="w-full bg-white/80 dark:bg-black/80 backdrop-blur-lg border-b border-gray-200/50 dark:border-gray-800/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div ref={headerRef}>
            <ProfileHeader
              profile={{
                ...userProfile,
                // Fixed: Use userProfile.cover_image_url instead of user.cover_image_url
      ...
      <div className={styles.profileContainer}>
        {/* Tab Navigation */}
        <div className="bg-white/80 dark:bg-black/80 backdrop-blur-lg rounded-2xl border border-gray-200/50 dark:border-gray-800/50 p-2 shadow-sm mb-6" ref={tabsRef}>
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex items-center justify-center flex-grow basis-0 min-w-0 px-2 py-2 sm:px-2.5 sm:py-2.5 rounded-lg sm:rounded-xl font-medium text-xs sm:text-sm transition-all duration-200 ${activeTab === tab.id
                    ? 'bg-[#EF3866] text-white shadow-md'
                    : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white'
                    }`}
                >
                  <Icon className="w-4 h-4" />
                  <span className="hidden sm:inline ml-1.5 text-xs sm:text-sm">{tab.label}</span>
                </button>
              );
            })}
          </div>
